"""
eval_model.py — Evaluate the fine-tuned BERT model on eval_data.csv.

Outputs
-------
  results/predictions_eval.xlsx      — Excel with actual vs predicted + confidence cols
  results/report_eval.png            — Classification report + confusion matrix image

Usage:
    python eval_model.py
    python eval_model.py --input path/to/custom.csv --output path/to/out.xlsx
"""
...
DEFAULT_INPUT  = os.path.join(BASE_DIR, "eval_data.csv")
OUTPUT_DIR     = os.path.join(BASE_DIR, "results")
DEFAULT_EXCEL  = os.path.join(OUTPUT_DIR, "predictions_eval.xlsx")
DEFAULT_REPORT = os.path.join(OUTPUT_DIR, "report_eval.png")
MAX_LENGTH     = 128
BATCH_SIZE     = 32
...
def save_excel(df_result: pd.DataFrame, path: str):
    with pd.ExcelWriter(path, engine="openpyxl") as writer:
        ...
        class_names = sorted(df_result["actual_category"].unique().tolist())
        report_dict = classification_report(y_true, y_pred, labels=class_names, output_dict=True)
        ...
        summary_rows.append({
            ...
            "Precision": "",
            "Recall":    "",
            "F1-Score":  round(accuracy_score(y_true, y_pred), 4),
            "Support":   len(y_true),
        })
...
def save_report(y_true, y_pred, class_names, report_path, source_file):
    """Save a PNG report: per-class metrics table + bar chart + confusion matrix."""
    ...
    cmap = LinearSegmentedColormap.from_list("emerald", ["#1e293b", "#10b981"])
    ...
    fig.suptitle(
        f"Support Ticket Classifier — Evaluation Report  ({os.path.basename(source_file)})",
...
# ─────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Evaluate fine-tuned BERT on eval_data.csv.")
    ...
    # ── Save Excel (2 sheets: Predictions + Summary) ─────────────────────
    print(f"\n[4/4] Saving outputs ...")
    ...
    # ── Save report ──────────────────────────────────────────────────────
    save_report(y_true, y_pred, class_names, args.report, args.input)
    print("=" * 60)
    print("\nDone!")
